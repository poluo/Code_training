...
int main(int argc, char const *argv[])
{
	...
	arrlist_print(test);
	arrlist_sort(test,ArrayList_int_compare,0,test->length-1);
	arrlist_print(test);
	getchar();
	return 0;
}
...
{
	ArrayList *tmp_arraylist = calloc(1,sizeof(ArrayList));
				  ...
				  unsigned int left,unsigned int right)
{
	...
	{
		if (compare_func(list->data[i],list->data[left])<0)
		{
	...
	arrlist_sort(list,compare_func,left,last-1);
	arrlist_sort(list,compare_func,last+1,right);
}
